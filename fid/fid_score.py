...
def compute_activations_for_images(img_batch, model, device):

    img_batch = torch.tensor(img_batch).to(device=device, dtype=torch.float32)
...
def compute_statistics_for_images(img_batch, model, device):

    activations = compute_activations_for_images(img_batch=img_batch, model=model, device=device)

    return compute_statistics(activations)
